...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  ...
  function setInfo() {
    localStorage.setItem(item.id, JSON.stringify({ time: time ? time : JSON.parse(localStorage.getItem(item.id)).time, selfService}))
      setFaster(false)
  }
  function getInfoTime() {
    return JSON.parse(localStorage.getItem(item.id)).time
  }
  ...
  return (
    <div className="Place">
      ...
      <footer className="Place__footer">
        <Link onClick={()=>{ setInfo() }} to={`/order/${area.id}/${item.id}`} className="Place__order">
          Оплатить {price}
        </Link>
      </footer>
    </div>
